...
lazy_static! {
    ...
    static ref SPACES: Regex = { Regex::new(r"[\r\n\f]+").unwrap() };
}
...
impl<'input, Input: CharStream<From<'input>>> Python3Lexer<'input, Input> {
    fn NEWLINE_action(
        ...
        if action_index == 0 {
            type NewLineToken<'input> =
                std::boxed::Box<antlr_rust::token::GenericToken<std::borrow::Cow<'input, str>>>;
            fn common_token<'input, Input: CharStream<From<'input>>>(
                recog: &mut <Python3Lexer<'input, Input> as Deref>::Target,
                ty: isize,
                text: &str,
            ) -> NewLineToken<'input> {
                ...
                recog.get_token_factory().create(
                    recog.input.as_mut(),
                    ty,
                    Some(text.to_string()),
            ...
            fn emit<'input, Input: CharStream<From<'input>>>(
                recog: &mut <Python3Lexer<'input, Input> as Deref>::Target,
                tok: NewLineToken<'input>,
            ) {
            ...
            let spaces = SPACES
                .replace_all(recog.get_text().as_ref(), "")
                .to_string();
