...
fn loss_sum(truths: &Tensor, predicts: &Tensor) -> f64 {
    let mut sum = 0.0;
    let len = truths.len();
...
fn main() {
    ...
    let truth = &mut Tensor::<Backend, 2>::from_floats(truth_floats, &device);
    let predict = Tensor::<Backend, 2>::from_floats(predict_floats, &device);
