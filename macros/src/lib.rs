...
pub fn auto_visitor(args: TokenStream) -> TokenStream {
    ...
    let path = env::current_dir().unwrap().join(PathBuf::from(format!("analysis/src/gen/{}visitor.rs", name.to_token_stream().to_string().to_lowercase())));
    ...
    let visitor_trait = visitor.items.iter().find_map(|t| match t { Item::Trait(t) => Some(t), _ => None }).unwrap();

    let functions = visitor_trait.items.iter().filter_map(|ti| match ti { TraitItem::Fn(f) => Some(f), _ => None }).collect::<Vec<_>>();

    // Enum cases that are always needed but are not listed in the trait
    let always_enum_cases = ["Terminal", "Whitespace"].iter().map(|c| syn::parse_str::<Ident>(c).unwrap());

    // Generate function and enumeration definitions
    let generated_enum_cases = functions.iter().map(|f| {
    ...
    }).chain(always_enum_cases).collect::<Vec<_>>();

    let generated_functions = functions.iter().zip(generated_enum_cases.iter()).map(|(f, e)| {
        let name = &f.sig.ident;
